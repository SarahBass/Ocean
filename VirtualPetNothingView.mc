...
//Text Movement
 ...
 /*     _                           _            _    
   ...
   //Draw graphics
   if (userSTEPS <= 3100){
    var water= waterPhase(userSTEPS, 0);
    water.draw(dc);
     var water1= waterPhase(userSTEPS,138);
    water1.draw(dc);
    var water2= waterPhase(userSTEPS,276);
    water2.draw(dc);
    if ((centerX*2)==454){
    var water3= waterPhase(userSTEPS,414);
    water3.draw(dc);
    }
   }else if (userSTEPS > 3100 && userSTEPS <= 4500 ){
    var water= waterPhase2(userSTEPS,1, 0, 3100);
    water.draw(dc);
   }else if (userSTEPS > 4500 && userSTEPS <= 6500 ){
    var water= waterPhase2(userSTEPS,1, 0, 4500);
    water.draw(dc);
    var water1= waterPhase2(userSTEPS,2, 100, 7500);
    water1.draw(dc);
   }else if (userSTEPS > 6500 && userSTEPS <= 9500 ){
    var water= waterPhase2(userSTEPS,0, 0, 6500);
    water.draw(dc);
    var water1= waterPhase2(userSTEPS,0, centerX, 8500);
    water1.draw(dc);
        var water2= waterPhase2(userSTEPS,0, 0, 9000);
    water2.draw(dc);
   }else{
   if (userSTEPS > 9500) {
    // Calculate step offset relative to 9500, repeating every 1000 steps
    var stepOffset = (userSTEPS - 9500) % 1000;
    
    // Adjust the parameters of waterPhase2 using stepOffset
    var water = waterPhase2( 9500, 3, -(centerX * 3) + today.sec + stepOffset, 9900);
    
    // Draw the water graphic
    water.draw(dc);
}
   }
   // var dog = dogPhase(today.sec,today.min); //userSTEPS or (today.sec*180) fix 15 and 16 and 17 to be higher
    //dog.draw(dc);
    var dog1 = dogPhase1(today.sec,today.min);
    dog1.draw(dc);

if (userSTEPS>=2000){
    var dog2 = dogPhase2(today.sec);
    dog2.draw(dc);
}  
...
function dogPhase1(seconds, minutes) {
    var screenHeightY = System.getDeviceSettings().screenHeight;
    var screenWidthX = System.getDeviceSettings().screenWidth;
    var venus2X = (110 * screenWidthX / 360);  // Initial X position
    var venus2Y = (280 * screenHeightY / 360); // Fixed Y position
    ...
    return dogARRAY[((minutes % 15 * 2)) + seconds % 2];
}
...
function spongePhase(seconds, minutes) {
    ...
    var spongeY = (200 * screenHeightY / 360); // Vertical position

    // If seconds is less than 15, move the dog based on seconds (horizontal)

    var spongeX = ((seconds % 20) * 20);  // Keep moving horizontally based on seconds
...
return spongeARRAY[minutes % 16];
}

function dogPhase2(seconds){
    //var screenHeightY = System.getDeviceSettings().screenHeight;
    //var screenWidthX = System.getDeviceSettings().screenWidth;
    ...
    // If seconds is less than 15, move the dog based on seconds (horizontal)
    venus2X = (seconds % 30) * 30;  // Keep moving horizontally based on seconds
    venus2Y = (seconds % 30) * 30;  // Keep moving horizontally based on seconds
...
function waterPhase(steps, position){
  var screenHeightY = System.getDeviceSettings().screenHeight;
  //var screenWidthX = System.getDeviceSettings().screenWidth;
